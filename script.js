...
document.querySelectorAll('.main-nav > ul > li').forEach(li => {
  ...
  trigger.addEventListener('click', (e) => {
    ...
    // If it's just an in-page anchor with no meaningful target beyond toggling, still allow default scroll
...
function applyStoredTheme() {
  try {
    const saved = localStorage.getItem('d16-theme');
...
themeToggle.addEventListener('click', () => {
  ...
  try { localStorage.setItem('d16-theme', next); } catch (e) {}
});
